...
/*!
 *\brief The VIP lite API for Convolution Neural Network application on MCU/DSP type of embedded environment.
...
typedef enum _vip_buffer_quantize_format_e {
	...
	/*! \brief The data is quantized with TF(affine) asymmetric format */
...
typedef enum _vip_buffer_memory_type_e {
	...
	*/
	...
	/*! \brief Create a VIP buffer from the Host pointer(logical or physical). */
...
typedef enum _vip_buffer_property_e {
	...
	/*!< \brief The data type for input/output buffer, the returned value is <tt>\ref vip_buffer_type_e</tt> */
	VIP_BUFFER_PROP_DATA_TYPE            = 8,
...
/*! \brief  Initial VIP Hardware and VIP lite software environment.
 ...
 *\param [in] the handle_size should be aligned to 64byte for easy flash CPU cache.
 *\param [out] buffer. vip lite buffer object.
 *\return <tt>\ref vip_status_e </tt>
 *\ingroup group_buffer
 *\version 1.1
...
/*! \brief Configure network property
...
/*! \brief. Create a vip_group object to run multiple tasks(network or node)
            and without interrupte between each task.
*\return <tt>\ref vip_status_e </tt>
